...
* [ReadFile]      : Lit le contenue d'un fichier, idéale pour les petits fichier sinon prendre stream
...
/**
* [ReadFile] 
* Desc   |                  | Retourne le contenu d'un fichier
* Entree | (String) | Un fichier
* Sortie | (String)         | retourne le contenue du fichier sinon retourne une erreur
*/
function ReadFile(file){
  return new Promise( (resolve,reject) => {
    fs.exists(file, (exists) => {
      if(!exists) { reject(`Le fichier : ${exists} n'existe pas`) }
      else { 
        fs.stat(file, (err, stats) => {
          if(err) { reject(err) }
